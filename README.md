...
||3. Unity의 각 오브젝트들의 관계에 대한 이해와 사용이 어려웠다.|3. 해결 방법 :  만드는 과정에서 결과Scene의 컴포넌트를 추가할 때 추가해야 할 스크립트를 잘 모르겠는 부분은 영현님이 필요한 script를 알려주셨고, 만든 Scene을 실행할 수 없었던 부분은 지연님이 build setting에서 추가해야 할 Scene을 알려주셔서 배우고 실행할 수 있었다. |
